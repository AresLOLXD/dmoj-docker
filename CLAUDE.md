...
- This stack assumes an external TLS-terminating proxy (e.g. Cloudflare Tunnel, Caddy) runs on the same host as this Docker Compose stack and forwards to `nginx` on `127.0.0.1:8080` (not the standard port 80 — this host's rootless podman can't bind host ports below 1024), setting `X-Forwarded-Proto: https`. `nginx` itself does not terminate TLS. Without such a proxy, the site is unreachable from outside the host — this is intentional (see M6 in the security audit), not a misconfiguration to revert.
